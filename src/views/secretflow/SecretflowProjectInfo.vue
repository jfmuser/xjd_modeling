...
import { computed, onMounted, reactive, ref, watch, defineEmits, onBeforeMount } from 'vue';
           ...
           currentProject?.nodes.forEach(item => {
                ...
                siteStore.otherSite.forEach(site => {
                    if (site.tDomainEngineList && site.tDomainEngineList.find(engine => engine.engine == '1')) {
                        const secretflowInfo = JSON.parse(site.tDomainEngineList.find(engine => engine.engine == '1').engineInfo)
        ...
        <ListContainerItem title="基本信息">
            <template #header-tool>
                <template v-if="FormType.EDIT === action">
                    <el-button type="text" @click="onSaveProjectBaseInfo">下一步
                    </el-button>
