...
<template>
    ...
    <div class="job-detail__top">
      ...
      <div class="job-param-output">
        ...
        <div class="job-output">
          ...
          <el-button :disabled="Object.keys(state.component).length === 0" type="primary" @click="onShowOutput"
            >查看作业输出
          </el-button>
        </div>
      </div>
  ...
  <OutputDialog
    ...
    :role="role"
    :party-id="partyId"
    :status="state.jobData.status"
    @close-dialog="closeOutputDialog"
    @download="handleDownloadDialog"
    @filter-logic="handleFilterLogic"
    @c-reporter="downloadFile"
  ...
  <DownloadReport
    ...
    :use-logic="state.useLogic"
    :table-data="state.variableMap"
    @download="handleBeforeDownload"
    @filter-logic="handleFilterLogic"
  />
