...
export default function (summary, quantilePR, isEvaluation) {
  ...
  const tableData = flattenToTable(
    summary,
    (subitem, metric_name, item, metric_namespace, index) => {
      if (index === 0) {
  ...
  if (quantilePR) {
    ...
    form.props.form.splice(1, 1, {
      ...
      props: {
        ...
        step: 0.05,
        value: 0.5,
        tip: '更新新分位数条件下的精度和召回率',
      },
    });
    each(tableData, (row) => {
      const item =
        quantilePR[row.metric_namespace] &&
        quantilePR[row.metric_namespace][`${row.metric_name}_quantile_pr`];
      if (item && item.meta) {
