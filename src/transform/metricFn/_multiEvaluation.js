...
export default function (metricData) {
  ...
  each(metricData, (curves, metric_namespace) => {
    ...
    each(curves, (curve) => {
      ...
      series.name =
        meta.curve_name + (meta.ordinate_name ? `_${meta.ordinate_name}` : '');
      options.push(series);
    });
    options = options.sort((a, b) => {
      if (
        a.name &&
        b.name &&
        b.name.toLowerCase().match('precision') &&
        a.name.toLowerCase().match('recall')
      ) {
        return 1;
      } else {
        return -1;
    ...
    setting.tooltip.formatter = (params) => {
      ...
      params.forEach((obj) => {
        ...
        if (obj.seriesName.toLowerCase().match('precision')) {
          str += `Precision(${obj.seriesName.replace(
            '_Precision',
            '',
          )}): ${value}<br>`;
        } else if (obj.seriesName.toLowerCase().match('recall')) {
          str += `Recall(${obj.seriesName.replace(
            '_Recall',
            '',
          )}): ${value}<br>`;
        }
