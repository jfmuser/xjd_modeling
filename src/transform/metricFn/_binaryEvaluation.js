...
export default function (metricData) {
  ...
  each(metricData, (originCurves, metric_namespace) => {
    ...
    each(pairGroup, ([pairType, curves]) => {
      const recallCurves = curves.find(
        (item) =>
          item.meta.metric_type === METRIC_TYPES.RECALL_BINARY_EVALUATION,
      );
      const precisionCurves = curves.find(
        (item) =>
          item.meta.metric_type === METRIC_TYPES.PRECISION_BINARY_EVALUATION,
      );
      thresholdsArr[recallCurves.meta.curve_name] =
        recallCurves.meta.thresholds;
      thresholdsArr[precisionCurves.meta.curve_name] =
        recallCurves.meta.thresholds;
