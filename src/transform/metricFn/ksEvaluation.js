...
const shortCurveName = (name) => {
  return (
    name &&
    name
      .replace(/(train_|validate_)/g, '')
      .replace(/(train\.|validate\.)/g, 'fold_')
  );
};
    ...
    arr.forEach((ksObj, ksIndex) => {
      ...
      if (thresholdValue || thresholdValue === 0) {
        for (const val of params) {
          if (
            val.seriesName.match(shortCurveName(ksObj.pairType)) &&
            (val.seriesName.match(ksObj.tpr) || val.seriesName.match(ksObj.fpr))
          ) {
            listCheck.push(val);
          }
        }
...
export default function (data) {
  ...
  each(data, (ksData) => {
    ...
    each(groupData, ([pair_type, data]) => {
      ...
      const serie1 = createSerie(
        shortCurveName(data1.meta.curve_name),
        data1.data,
        { pairType: pair_type },
      );
      const serie2 = createSerie(
        shortCurveName(data2.meta.curve_name),
        data2.data,
        { pairType: pair_type },
      );
