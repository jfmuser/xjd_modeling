...
const fn = (response, params) => {
  ...
  const {
    data_split_count_info: count_info,
    data_split_label_info: label_info,
    data_split_ratio_info: ratio_info,
    stratified,
  } = response.data_split.meta;
  const tableData = [];
            ...
            if (i === 0) {
              if (h.prop !== 'original') {
                total[h.prop] =
                  count_info[h.prop] +
                  '(' +
                  (ratio_info[h.prop] * 100).toFixed(4) +
                  '%)';
              } else {
