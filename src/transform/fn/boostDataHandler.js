...
function boostDataHandler({ responseData, role, partyId, outputType }) {
  ...
  const { featureNameFidMapping, featureImportances, trees, classes, treeDim } =
    responseData;
  ...
  each(trees, (item, index) => {
    ...
    allTreesLineSeriesData[i].push([
      parseInt(index / allTreesLen),
      item.tree.length,
    ]);
  ...
  if (featureImportances && featureImportances.length > 0) {
    ...
    featureImportances.forEach((item, index) => {
      ...
      const variable =
        itemRole === role && itemPartyId === partyId
          ? featureNameFidMapping[item.fid]
          : item.fid + '';
      variableData[reverseIndex] = variable;
      const variableLen = variable
        ? variable.toString().length
        : (typeof variable).length;
