...
function deepJoin(list, str = '|') {
  ...
  list.forEach((item) => {
    res =
      res +
      ((res ? str : '') + (Array.isArray(item) ? deepJoin(item, str) : item));
  });
  return res;
}
...
function compareWith(val, list) {
  ...
  if (compare) {
    for (let i = 0; i < list.length; i++) {
      const item = list[i];
      if (
        (Array.isArray(item) && compareWith(val, item) >= 0) ||
        compare[0] === item
      ) {
        return i;
      }
...
function toCurves(type, val, nameSpace, pos) {
  ...
  content.property[label + '_' + nameSpace] = extendsMetrics(
    content.property[label + '_' + nameSpace],
    nameSpace,
    val,
  );
...
function toTheType(variable, nameSpace, type = 'others') {
  const pos =
    type !== 'others'
      ? compareWith(variable.toString().toLowerCase(), DEF['$' + type])
      : 0;
  const opera =
    ['curves', 'loss', 'dbi'].indexOf(type) >= 0 ? 'curves' : 'others';
  if (pos >= 0) {
    if (opera === 'others') {
