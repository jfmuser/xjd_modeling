...
import { getMetricData, getMetrics } from '@/apis/workspace/chart.api';
import { formatFloat, deepClone } from '@/utils';
import metricsArrange from './metricsArrange';
import isFunction from 'lodash/isFunction';

const makeMetricsDataRequest = (
  name,
  namespace,
  partyId,
  role,
...
export const getAllMetricsRequest = (
  metrics,
  partyId,
  role,
  componentName,
  ...
  each(metrics, (metricsNames, namespace) => {
    each(metricsNames, (name) => {
      allMetricsRequest.push(
        makeMetricsDataRequest(
          name,
          namespace,
          partyId,
          role,
...
const createItem = (variable, weight) => ({
  variable,
