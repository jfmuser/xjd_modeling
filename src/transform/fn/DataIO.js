...
import { createHeader, wrapGroupComponent } from './common';
...
const wrapComponent = wrapGroupComponent;
...
function dataIOHandler(modelData) {
  ...
  if (isExistImputerParams) {
    each(
      combineTo(
        imputerParam.missingValueRatio,
        imputerParam.missingReplaceValue,
      ),
      iteratee(imputerData),
    );
  ...
  if (isExistOutlierParams) {
    each(
      combineTo(
        outlierParam.outlierValueRatio,
        outlierParam.outlierReplaceValue,
      ),
      iteratee(outlierData),
    );
