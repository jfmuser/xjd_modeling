...
export default function (
  ...
  if (!isEmpty(data)) {
    ...
    for (const key in data) {
      ...
      let min = -9999;
      const iterationArr =
        data[key].ivArray.length > 0
          ? data[key].ivArray
          : data[key].splitPoints;
      iterationArr.forEach((item, index, self) => {
        ...
        const event_ratio = multiply(
          formatFloatWithDefault(
            data[key].eventRateArray[index],
            Currentrole,
            skipStatic,
          ),
          100,
        );
        const non_event_ratio = multiply(
          formatFloatWithDefault(
            data[key].nonEventRateArray[index],
            Currentrole,
            skipStatic,
          ),
          100,
        ...
        tableData.push({
          ...
          event_count: formatFloatWithDefault(
            data[key].eventCountArray[index],
            Currentrole,
            skipStatic,
          ),
          event_ratio: event_ratio ? event_ratio.toFixed(4) + '%' : '',
          non_event_count: formatFloatWithDefault(
            data[key].nonEventCountArray[index],
            Currentrole,
            skipStatic,
          ),
          non_event_ratio: non_event_ratio
            ? non_event_ratio.toFixed(4) + '%'
            : '',
          woe: formatFloatWithDefault(
            data[key].woeArray[index],
            Currentrole,
            skipStatic,
          ),
          iv: formatFloatWithDefault(
            data[key].ivArray[index],
            Currentrole,
            skipStatic,
          ),
        });
        formatterArr.push({
          ...
          event_ratio: formatFloatWithDefault(
            data[key].eventRateArray[index],
            Currentrole,
            skipStatic,
          ),
          ...
          non_event_ratio: formatFloatWithDefault(
            data[key].nonEventRateArray[index],
            Currentrole,
            skipStatic,
          ),
      ...
      stackBarData[key] =
        data[key].eventCountArray.length > 0 &&
        data[key].nonEventCountArray.length > 0
          ? eventSeries
          : {};
      const woeSeries = [];
      woeSeries.push({
      ...
      sourceData.push({
        ...
        monotonicity: skipStatic
          ? '-'
          : Currentrole === 'host'
          ? '-'
          : data[key].isWoeMonotonic
          ? data[key].isWoeMonotonic.toString()
          : 'false',
        partyid: partyId,
        role,
    ...
    sourceData.sort((a, b) => {
      ...
      if (matchIndexA && matchIndexB) {
        if (
          parseInt(a.variable.match(/[0-9]+$/)[0]) >
          parseInt(b.variable.match(/[0-9]+$/)[0])
        ) {
          return 1;
        } else {
          return -1;
    ...
    for (const val of sourceData) {
      for (let i = 0; i < options.length; i++) {
        if (
          val.variable === options[i].label ||
          val.variable === options[i].value
        ) {
