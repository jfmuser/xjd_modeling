...
const filterObjArray = (target, key, host) => {
  ...
  result = target.reduce((item, next) => {
    countObj[next[key]]
      ? null
      : (countObj[next[key]] = true && item.push(next));
    return item;
  }, []);
  if (host) {
...
const getFormSettingByRole = (role, hostId) => {
  ...
  if (role === 'guest') {
    ...
    if (hostId.length > 0) {
      formSetting.push(
        createFormComponent('f-select', 'f-selection', {
          options: [
            createOption('guest'),
            createOption(
              'host',
              hostId.length > 0
                ? hostId.map((val) => ({ label: val, value: val }))
                : 'host',
            ),
          ],
          multiple: false,
...
const getTableHeaderSettingByRole = (role, hostId, guest, host) => {
  ...
  if (role === 'guest') {
    ...
    header['header'] = [{ type: 'index', label: 'index' }].concat(
      filterObjArray(commonHeader, 'prop', host[0]),
    );
...
const fn = (modelData, metricsData, partyId, role) => {
  ...
  // let hostId = []
  if (results) {
    const { guestBody, guestHeader, hostBody, hostHeader, hostId } =
      handleSelectionData(modelData.data.data, partyId, role);
    ...
    return [
      {
        ...
        options: [
          ...
          },
          {
            type: 'table',
            props: {
              header: getTableHeaderSettingByRole(
                role,
                hostId,
                guestHeader,
                hostHeader,
              ),
              data: getTableDataSettingByRole(
                role,
                hostId,
                guestBody,
                hostBody,
