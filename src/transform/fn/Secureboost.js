...
function modelTreeData(modelData, partyId, role) {
  ...
  const impleColor =
    labelSelection.length > 0 ? {} : `rgba(${basicColor[0]},1)`;
  ...
  trees.forEach((item, index) => {
    ...
    if (Array.isArray(impleToSelection)) {
      ...
      impleColor[key] = `rgba(${
        basicColor[parseFloat(key) % basicColor.length]
      },1)`;
      const name = key + '_' + (impleToSelection[key].length - 1);
      const objList = Object.keys(impleToTrees).filter((item) =>
        item.match(key + '_'),
      );
...
function setTreeDatasDiagram(data, bcolor) {
  if (data && Object.keys(data).length > 0) {
    ...
    options.series.leaves.label.backgroundColor = bcolor.replace(
      ',1)',
      ',0.6)',
    );
    return options;
...
function setGraphOptions(modelData, partyId, role) {
  ...
  if (Array.isArray(treesData.treeLines)) {
    treesData.treeLines = setTreelines(
      treesData.treeLines,
      treesData.maxmium,
      treesData.colors,
    );
...
function transfromTreeData(props) {
  const opts = handleTreeData(
    props.tree,
    props.role,
    props.partyId,
    props.featureNameFidMapping,
    props.splitMaskdict,
    props.missingDirMaskdict,
    props.type,
    props.workMode,
    props.treePartyId,
  );
...
function transformImportance(responseData) {
  if (
    responseData.featureImportances &&
    responseData.featureImportances.length > 0
  ) {
    const tBody = [];
    each(deepClone(responseData.featureImportances), (item) => {
      tBody.push(
        Object.assign({}, item, {
          gain:
            (item.main && item.main === 'split') || !item.main
              ? item.importance2
              : item.importance,
          importance:
            item.main && item.main === 'gain'
              ? item.importance2
              : item.importance,
        }),
      );
...
function importanceSetting(responseData, role) {
  ...
  return !asyncSetting.type
    ...
    : [...fromComponent, asyncSetting];
}

function mappingSetting(responseData, partyId, role) {
  if (
    responseData.featureNameFidMapping &&
    Object.keys(responseData.featureNameFidMapping).length > 0
  ) {
...
async function metricsTransform(
  group,
  metricsData,
  partyId,
  role,
...
const fn = async (
  ...
  if (modelData.data.meta) {
    ...
    if (
      role === 'host' &&
      !outputType.toLowerCase().match('homo') &&
      !workMode.toLowerCase().match('mix')
    ) {
