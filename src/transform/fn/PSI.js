...
const getPsiChartSetting = (xAxis, expXa, actXa, psiXa) => {
  ...
  xs.forEach((x, index) => {
    ...
    common.yAxis[0].axisLabel.formatter = (value) => {
      return `${accMul(value, 100, biggest)} %`;
    };
    const biggestPsi = getBiggest(psiXa[x]);
    common.yAxis[1].max = biggestPsi;
    common.yAxis[1].interval = biggestPsi / 5;
    common.yAxis[1].axisLabel.formatter = (value) => {
      return `${accMul(value, 1, biggestPsi)}`;
    };
  ...
  return [
    {
      component: defineAsyncComponent(() =>
        import('@/components/ComponentGroup/index.vue'),
      ),
      options: [
        {
          type: 'table',
