...
function binningHandler(responseData, metricsData, partyId, crole) {
  ...
  const { skipStatic } =
    ...
    responseData.data.meta.meta_data;
  ...
  if (role === 'guest') {
    if (hostPartyId.length > 0) {
      form1.push(
        createFormComponent('f-select', 'tableSelection', {
          options: [
            createOption('guest'),
            createOption(
              'host',
              hostPartyId.map((val) => ({ label: val, value: val })),
            ),
          ],
          multiple: true,
        }),
    ...
    if (hostPartyId.length > 0) {
      options = [
        ...
        createOption(
          'host',
          hostPartyId.map((val) => ({ label: val, value: options4form2[val] })),
        ),
      ];
    } else {
    ...
    form2 = [
      ...
      createFormComponent('f-select', 'f-select', {
        options: options,
        supportFilter: true,
      }),
    ];
  ...
  if (crole === 'guest') {
    form2.push(
      createFormComponent('tslider', 'tslider', {
        label: 'woe range',
        range: true,
        step: 0.001,
