...
export default function (
  ...
  const pushData = ({
    ...
    const splitValue =
      outputType !== modelNameMap.homoBoost
    ...
    if (
      ...
      name += `${variable} <= ${
        splitValue !== undefined ? splitValue : node.bid || 0
      }`;
    ...
    if (
      ((node.isLeaf && role === 'guest') ||
        (node.isLeaf && outputType === modelNameMap.homoBoost)) &&
      !rootNode
    ) {
    ...
    if (modelNameMap.homoBoost !== outputType) {
      name +=
        workmode.toLowerCase() === 'mix' &&
        role.toLowerCase().match('guest') &&
        node.leftNodeid === -1 &&
        node.rightNodeid === -1 &&
        depth === 0
          ? 'HOST' + (defPartId ? ':' + defPartId : '')
          : `${nodeRole && nodeRole.toUpperCase()} ${
              nodePartyId ? ':' + nodePartyId : defPartId ? ':' + defPartId : ''
            }`;
    }
    arr.push({
      treeid: node.id,
  ...
  const treeData = [];
  const mixecheck = pushData({
    arr: treeData,
    node: rootNode,
    rootNode:
      workmode.toLowerCase() === 'mix' && role.toLowerCase().match('guest')
        ? 'Multi nodes'
        : '',
  });
  let treeWidth = 0;
    ...
    const insertChild = (arr, leftNodeId, rightNodeId, depth) => {
      ...
      if (leftNode) {
        ...
        pushData({
          arr,
          node: leftNode,
          grandChildrenArr: leftGrandChildrenArr,
          depth,
        });
        treeWidth += insertChild(
          leftGrandChildrenArr,
          leftGranChildId,
          rightGranChildId,
          depth,
        );
      ...
      if (rightNode) {
        ...
        pushData({
          arr,
          node: rightNode,
          grandChildrenArr: rightGrandChildrenArr,
          depth,
        });
      } else if (leftNode) {
        treeWidth += defWidth;
