...
export default function (data, partyId, role) {
  ...
  results.forEach((item) => {
    ...
    guestColNames.forEach((variable) => {
      const index = guestBody.findIndex(
        (guestItem) => variable === guestItem.variable,
      );
      if (index >= 0) {
        guestBody[index][filterName] =
          featureValues[variable] === undefined
            ? '-'
            : formatFloat(featureValues[variable]);
      }
    ...
    if (hostFeatureValues && hostFeatureValues.length > 0) {
      for (let i = 0; i < hostFeatureValues.length; i++) {
        if (hostFeatureValues[i].featureValues) {
          ...
          for (const val of mid) {
            val[filterName] =
              fe[val.variable] === undefined
                ? '-'
                : formatFloat(fe[val.variable]);
          }
        }
      }
      ...
      for (let i = 0; i < hostLeftCols.length; i++) {
        if (hostLeftCols[i].leftCols) {
          const mid = hostBody[i];
          mid.forEach((item) => {
            item[filterName + '_disable'] =
              !hostLeftCols[i].leftCols[item.variable];
          });
        }
      }
    }
