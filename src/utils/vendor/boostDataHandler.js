...
export default function ({ responseData, output, role, partyId, outputType }) {
  ...
  if (treeDim < 3) {
    ...
    trees.forEach((item, index) => {
      allTreesOverviewData[index % treeDim].push(
        handleTreeData(
          item.tree,
          role,
          partyId,
      ...
      allTreesLineSeriesData[index % treeDim].push([
        Number.parseInt(index / treeDim),
        item.tree.length,
      ]);
    });
  ...
  if (
    ...
    const final = JSON.parse(
      JSON.stringify(responseData.featureNameFidMapping),
    );
    const tBody = [];
    const tHeader = [
      { label: 'variable', prop: 'variable' },
      { label: 'anonym in guest', prop: 'featureIndex' },
    ];
