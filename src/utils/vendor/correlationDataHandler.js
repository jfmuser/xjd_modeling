...
export default function (data, role) {
  ...
  if (role === 'guest') {
    final.localHeader =
      data.allNames.length > 1
        ? data.allNames[0].names
        : data.allNames[0].names;
    final.otherHeader = data.allNames.length > 1 ? data.allNames[1].names : [];
  } else {
    final.localHeader =
      data.allNames.length > 1
        ? data.allNames[1].names
        : data.allNames[0].names;
  ...
  for (let i = 0; i < final.localHeader.length; i++) {
    for (let j = 0; j < final.localHeader.length; j++) {
      ...
      final.corr[final.localHeader[i]][final.localHeader[j]] =
        data.localCorr[i * final.localHeader.length + j];
    }
  }
  ...
  if (final.otherHeader.length > 0) {
    for (let i = 0; i < final.localHeader.length; i++) {
      for (let j = 0; j < final.otherHeader.length; j++) {
        ...
        final.corr[final.localHeader[i]][final.otherHeader[j]] =
          data.corr[i * final.otherHeader.length + j];
      }
    }
  }
  if (!single) {
