...
export default function ({
  ...
  curve_name =
    curve_name && curve_name.replace(/(train\.|validate\.)/g, 'fold_');
  ...
  if (metric_type === metricTypeMap.dataIOText) {
    ...
    metric_type === metricTypeMap.Accuracy ||
    ...
    if (metric_type === metricTypeMap.Gain) {
      ...
      outputData.tooltip.formatter = curveFormatter(
        'Threshold',
        'Gain',
        null,
        thresholds,
    ...
    for (let i = 0; i < evaluationOutputList.length; i++) {
      ...
      if (
        item.type &&
        item.type === type &&
        item.nameSpace === metric_namespace
      ) {
        if (
          metric_type === metricTypeMap.RecallMulti ||
          metric_type === metricTypeMap.PrecisionMulti
        ...
        if (
          ...
          if (metric_type === metricTypeMap.Gain) {
            item.data.tooltip.formatter = curveFormatter(
              'Threshold',
              'Gain',
              item.legendData,
              item.thresholdsArr,
            ...
            item.data.tooltip.formatter = outputData.tooltip.formatter = (
              params,
            ) => {
              let str = '';
              const xValue = params[0].axisValue;
            ...
            item.data.tooltip.formatter = (params) => {
              ...
              params.forEach((obj, index) => {
                if (obj.seriesType !== 'line' || obj.color !== hideColor) {
                  if (item.legendData[index].isActive !== false) {
                    const value = Array.isArray(obj.data)
                      ? obj.data[1]
                      : obj.data;
                    str += `${obj.seriesName}: ${value}<br>`;
                  }
              ...
              params.forEach((obj, index) => {
                if (obj.seriesType !== 'line' || obj.color !== hideColor) {
                  ...
                  if (items && items.isActive !== false) {
                    const xValue =
                      item.thresholdsArr[obj.seriesIndex][obj.dataIndex];
                    if (xValue || xValue === 0) {
    ...
    metric_type === metricTypeMap.RecallBinary ||
    metric_type === metricTypeMap.PrecisionBinary
  ) {
    let dataObj = {};
                ...
                if (obj.seriesType !== 'line' || obj.color !== hideColor) {
                  if (item.legendData[index].isActive !== false) {
                    const thresholdValue =
                      item.thresholdsArr[index][obj.dataIndex];
                    if (thresholdValue || thresholdValue === 0) {
                      str += `Thresholds(${obj.seriesName}): ${thresholdValue}<br>`;
                    }
                    const value = Array.isArray(obj.data)
        ...
        for (let j = 0; j < item.data.series.length; j++) {
          ...
          if (curve.pair_type && curve.pair_type === pair_type) {
            ...
            if (/_tpr$/g.test(curve.name)) {
              ...
              item.legendData[legendIndex].unshift({
                color: curveColor[legendIndex],
                text: curve_name,
              });
            }
    ...
    metric_type === metricTypeMap.Upload ||
    metric_type === metricTypeMap.Download
  ) {
    type = 'text';
    outputData = `${data[0][0]}: ${data[0][1]}`;
    ...
    metric_type === metricTypeMap.F1Score ||
    metric_type === metricTypeMap.ConfusionMat
  ) {
    ...
    obj = obj.data || [];
    obj = ConfusionMatExchange(
      obj,
      meta,
      metric_namespace,
      metric_name.replace(/(train_|validate_)/g, ''),
    );
    outputData = obj;
...
function PSIExchange(PSIinstance, meta, nameSpace, effect) {
  ...
  const biggestY0 = getBiggest([
    ...meta.expected_percentage,
    ...meta.actual_percentage,
  ]);
  pic.yAxis[0].max = biggestY0;
  pic.yAxis[0].interval = biggestY0 / 5;
  pic.yAxis[0].axisLabel.formatter = (value) => {
    return `${accMul(value, 100, biggestY0)} %`;
  };
  pic.yAxis[1].name = 'PSI';
  const biggestY1 = getBiggest(meta.psi_scores);
  pic.yAxis[1].max = biggestY1;
  pic.yAxis[1].interval = biggestY1 / 5;
  pic.yAxis[1].axisLabel.formatter = (value) => {
    return `${accMul(value, 1, biggestY1)}`;
  };
  ...
  const biggest2Y0 = getBiggest([
    ...meta.expected_percentage,
    ...meta.actual_percentage,
  ]);
  quantilePic.yAxis[0].max = biggest2Y0;
  quantilePic.yAxis[0].interval = accDivCoupon(biggest2Y0, 5);
  quantilePic.yAxis[0].axisLabel.formatter = (value) => {
    return `${accMul(value, 100, biggest2Y0)} %`;
  };
  quantilePic.yAxis[1].name = 'event_ratio';
  const biggest2Y1 = getBiggest([
    ...meta.train_pos_perc,
    ...meta.validate_pos_perc,
  ]);
...
function getBiggest(vals) {
  ...
  let FixedTo =
    fx.length > 1 ? (fx[1].match(/^0+/) ? fx[1].match(/^0+/)[0].length : 0) : 0;
  if (FixedTo) {
    FixedTo += 2;
  } else {
    FixedTo = 2;
  ...
  while (midd < biggest) {
    midd = parseFloat(
      (midd + accDivCoupon(1, Math.pow(10, FixedTo))).toFixed(FixedTo),
    );
  }
  biggest = parseFloat(
    (
      (Math.ceil((midd * Math.pow(10, FixedTo)) / 5) * 5) /
      Math.pow(10, FixedTo)
    ).toFixed(FixedTo),
  );
  if (biggest === 0) {
...
function accMul(arg1, arg2, lens) {
  try {
    let m = 0;
    let len =
      lens.toString().split('.').length > 1
        ? lens.toString().split('.')[1].length
        : 0;
    if (len < 2) len = 2;
