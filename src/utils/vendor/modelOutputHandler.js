...
export default function ({ outputType, responseData, role, partyId }) {
  ...
  if (
    ...
    const isExistImputerParams =
      imputerParam &&
      imputerParam.missingReplaceValue &&
      Object.keys(imputerParam.missingReplaceValue).length > 0;
    const isExistOutlierParams =
      outlierParam &&
      outlierParam.outlierReplaceValue &&
      Object.keys(outlierParam.outlierReplaceValue).length > 0;
    ...
    outputType === modelNameMap.homoLR ||
    ...
    if (data) {
      ...
      output = Object.assign(
        handleSelectionData(responseData, partyId),
        output,
      );
    } else {
      output.isNoModelOutput = true;
    ...
    if (
      ...
      const middleData = handleBinningData(
        data,
        responseData.header,
        'guest',
        guestPartyId,
      ...
      for (const key in hostData) {
        hostMiddle.push(
          handleBinningData(
            hostData[key].binningResult,
            responseData.header,
            'host',
            hostData[key].partyId,
            hostData[key].role,
            role,
          ),
        );
