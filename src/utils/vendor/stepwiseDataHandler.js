...
export default function stepwiseDataHandler(
  originDataList,
  data,
  meta,
  role,
  ...
  if (!role.toLowerCase().match('host')) {
    ...
    const theader = [
      {
        label: role.toLowerCase().match('guest')
          ? 'Intercept Only'
          : meta.score_name.toUpperCase(),
        prop: 'intercept_only',
      },
      {
        ...
        intercept_only: role.toLowerCase().match('guest')
          ? meta.fit_intercept
            ? meta.direction === 'forward'
              ? formatFloat(meta.loss) || '-'
              : '-'
            : '-'
          : formatFloat(meta.current_ic_val) || '-',
        i_a_c: role.toLowerCase().match('guest')
          ? '-'
          : formatFloat(meta.loss) || '-',
      },
    ];
    ...
    if (
      ...
      meta.name.toLowerCase().match('stepwise_0')
    ) {
    ...
    if (role.toLowerCase().match('arbiter')) {
      ...
      const stepNum =
        originDataList.steps.length > 0
          ? originDataList.steps[originDataList.steps.length - 1].name.match(
              /[0-9]+/,
            )[0]
          : -1;
    ...
    const theader = role.toLowerCase().match('guest')
      ? [
          ...
          },
          {
            label:
              'Anonym in ' +
              (role.toLowerCase().match('host') ? 'guest & ' : 'host & ') +
              'arbiter',
  ...
  for (let i = 0; i < summary.tables.length; i++) {
    ...
    if (
      summary.tables[i].tableName === 'Analysis of Maximum Likelihood Estimates'
    ) {
      hasAomle = true;
      summary.tables[i].tbody = tbody;
    }
