...
export default class Json2Csv {
      ...
      if (typeof field === 'object') {
        if (typeof field.value === 'string') {
          return {
            label: field.label,
            value: (row) =>
              getProp(row, field.value, this.options.defaultValue),
          };
        }
  ...
  processRow(row) {
    ...
    const processedRow = this.options.fields.map((field) =>
      this.processValue(field.value(row)),
    );
    if (
      this.options.includeEmptyRows &&
      processedRow.every((row) => row === undefined)
    ) {
      return undefined;
    }
  ...
  processData(data) {
    return fastJoin(
      data.map((item) => this.processRow(item)).filter((row) => row),
      this.options.eol,
    );
  }
  parse(data, order) {
    ...
    const csv =
      header + (header && rows ? this.options.eol : '') + rows || 'no data';
    return csv;
  }
}
