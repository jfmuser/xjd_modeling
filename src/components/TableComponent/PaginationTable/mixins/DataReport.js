  ...
  for (const val of data) {
    ...
    for (const key in val) {
      if (key in exchange) {
        ...
        mid[midKey] =
          !val[key].toString().toLowerCase().match('total') &&
          exchange[key].format
            ? exchange[key].format(val)
            : val[key];
        if (
          mid[midKey] === '' ||
          mid[midKey] === null ||
          mid[midKey] === 'null' ||
          mid[midKey] === undefined
        ) {
          mid[midKey] = ' ';
        }
      }
...
function combineData(data, header, exporName) {
  ...
  for (const key in data) {
    res[key] = toReportable(
      header[key] || header['default'],
      data[key],
      exporName,
    );
  }
...
const dataRequest = {
  methods: {
          ...
          if (Array.isArray(val)) {
            ...
            for (let i = 0, l = val.length; i < l; i++) {
              ...
              if (!c[TO]) {
                ...
                item[implying] &&
                item[implying] &&
                item[implying].match(c[ORIGIN]) &&
                item[implying] === val[ORIGIN]
              ) {
    ...
    tablesHeaderTo(allHeaders, allDatas) {
      ...
      if (Array.isArray(allDatas)) {
        properties =
          allHeaders.disable || allHeaders.disabled
            ? Object.keys(allHeaders.disable || allHeaders.disabled)
            : [];
      } else {
        properties = Object.keys(allDatas);
      }
      res.default = toOneLevel(
        Array.isArray(allHeaders) ? allHeaders : allHeaders.header,
      );
      for (const val of properties) {
        res[val] = toOneLevel(
          this.getHeaderList(
            Array.isArray(allHeaders) ? { header: allHeaders } : allHeaders,
            val,
          ),
        );
      }
    ...
    getTableFiltersValue(imply, compare) {
      let resData = Array.isArray(this.data)
        ? deepClone(this.currentDatas)
        : deepClone(this.data);
    ...
    checkFormatData(dataList) {
      if (Array.isArray(dataList)) {
        dataList = this.$refs.originTable.checkData(
          dataList,
          Array.isArray(this.header) ? this.header : this.header.header,
        );
        dataList = this.sortMethod(
          dataList,
        ...
        for (const val in dataList) {
          dataList[val] = this.$refs.originTable.checkData(
            dataList[val],
            Array.isArray(this.header) ? this.header : this.header.header,
          );
          dataList[val] = this.sortMethod(
            dataList[val],
            this.currentSortColumn,
            this.currentOrder,
          );
    ...
    checkinDataWithFilters(compare) {
      // this.filterForAllProperty(deepClone(this.data), algorithmUtil).length > 0
    },

    allSteps(args) {
      if (
        this.export &&
        args.needExport.join('|').match(this.export + '.csv')
      ) {
