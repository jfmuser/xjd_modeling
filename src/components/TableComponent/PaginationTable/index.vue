<template>
  <section v-loading="tableLoading" class="ctable__container">
    ...
    <div
      v-if="!outerPagination"
      :class="{ ctable_page_right: paginationRight }"
      class="ctable__pagination"
    >
      <el-pagination
        v-show="isNotNoData"
        ref="pagination"
        ...
        :page-size="
          pageSize < 0 || typeof pageSize === 'string'
            ? actualCurrentTotal
            : pageSize
        "
...
import OriginTable from '../OriginTable/index.vue';
...
export default {
  ...
  methods: {
    ...
    getList(obj, pro) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        for (let j = 0; j < res.length; j++) {
          if (
            res[j][val.property] === undefined ||
            !res[j][val.property]
              .toString()
              .match(new RegExp('(' + val.filter.join('|') + ')'))
          ) {
            res.splice(j, 1);
            j--;
          }
    ...
    setProperty(value) {
      ...
      this.currentDatas = this.getList(
        this.filterData(
          Array.isArray(this.data)
            ? [...this.data]
            : this.originGetList(this.data, this.property),
        ),
        this.dataFilter,
      );
      this.oldCurrentDatas = [];
      this.currentHeaders = this.filterHeader(
    ...
    filterHeader(origin, list) {
      if (!list || Object.keys(list).length === 0) {
        ...
        for (const item of origin) {
          if (
            item.pageFixed ||
            list.indexOf(item.label) >= 0 ||
            list.indexOf(item.prop) >= 0
          ) {
            res.push(item);
          }
        }
