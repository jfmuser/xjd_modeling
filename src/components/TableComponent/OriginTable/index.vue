...
import clink from '../CustomColumn/Linking/index.vue';
import ceditor from '../CustomColumn/Editor/index.vue';
import cweight from '../CustomColumn/Weight/index.vue';
...
export default {
  ...
  methods: {
    ...
    eachColumn(h, attrs) {
      ...
      const variable = {
        ...
        on: (() => {
          ...
          for (const key in this.$attrs) {
            if (key.match(/^column-/)) {
              res[key.replace('column-', '')] = this.$attrs[key];
            }
          }
          return res;
        })(),
      ...
      return h(ElTableColumn, variable, childEle);
    },
    columns(h, columns) {
      const cols = [];
    ...
    table(h) {
      ...
      return h(ElTable, variable, this.columns(h, this.header));
    },
  },
  render() {
    return this.table(hh);
  },
};
</script>
...
<style lang="scss">
.ctable__header-cell {
  background-color: #4376ff14 !important;
  color: #6a6c75;
  padding: 3px 0px;
  font-size: 0.86rem;
  .caret-wrapper {
