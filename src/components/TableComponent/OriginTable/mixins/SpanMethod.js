...
function totalSpanMethod(row, column, list, end) {
  if (
    row[column.property] &&
    row[column.property].toString().match(/(total|Total|summary|Summary)/)
  ) {
    let colOrigin = 1;
    for (const val of list) {
      if (!val.prop || !row[val.prop]) {
        ...
        row[val.prop].toString().match(/(total|Total|summary|Summary)/)
      ) {
        break;
      }
    }
...
const SpanMethods = {
  methods: {
    ...
    doCombine({ row, column, rowIndex, columnIndex }) {
      ...
      if (
        rowIndex > 0 &&
        compareRows(ctd[rowIndex - 1], ctd[rowIndex], compared)
      ) {
        return {
          rowspan: 0,
          colspan: 0,
        ...
        while (
          ctd[rowIndex + combineRowCount] &&
          compareRows(
            ctd[rowIndex + combineRowCount],
            ctd[rowIndex],
            compared,
          ) &&
          !ctd[rowIndex + combineRowCount]._total
        ) {
          combineRowCount++;
        }
        return {
