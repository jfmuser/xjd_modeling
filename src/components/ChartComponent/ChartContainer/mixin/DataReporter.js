...
const dataReporter = {
  ...
  methods: {
    ...
    allSteps(args) {
      ...
      const mid = checklist.match(this.export + '.png')
        ? this.getPicture()
        : null;
      const detail =
        checklist.match(this.export + '_detail') && this.detail
          ? this.getPicData()
          : null;
      this.$emit(
        'reporter',
    ...
    getVariableMap() {
      ...
      if (!Array.isArray(this.options) || this.options.length > 0) {
        variableMap = Array.isArray(this.options)
      ...
      for (const key in variableMap) {
        const items = Array.isArray(variableMap[key])
          ? variableMap[key]
          : [variableMap[key]];
