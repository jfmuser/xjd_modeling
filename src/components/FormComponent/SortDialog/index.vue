<template>
  <div class="sortDialog__container">
    <div class="sortDialog__content">
      <slot>
        <el-icon @click.stop="showDialog"><sort /></el-icon>
      </slot>
    </div>
    <div v-show="dialog" class="sortDialog__dialog" @click.stop>
      <header class="sortDialog__header">Sort by</header>
      <search
        :input-class-name="'sortDialog__input'"
        class="sortDialog__search"
        @search="searching"
      />
      <main class="sortDialog__main">
        <checkbox-group
          ...
          @change="sortChange"
        />
        <div v-show="currentOptions.length === 0" class="sortDialog__box">
          无数据
        </div>
      </main>
      <footer class="sortDialog__footer">
        <div class="sortDialog__left">
          <el-icon @click="sortMethod('ascending')"><top /></el-icon>
          <el-icon @click="sortMethod('descending')"><bottom /></el-icon>
        </div>
        <el-button
          :disabled="btnDisabled"
          class="sortDialog__right"
          @click="sorting"
          >OK</el-button
        >
      </footer>
...
export default {
  name: 'SortDialog',
  components: {
    checkboxGroup: () => import('../Checkbox/index.vue'),
    search: () => import('../Searching/index.vue'),
  },
  props: {
    options: {
      type: Array,
