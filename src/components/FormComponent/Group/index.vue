...
const ccheckbox = defineAsyncComponent(() => import('../Checkbox/index.vue'));
const cradio = defineAsyncComponent(() => import('../Radio/index.vue'));
const ceditor = defineAsyncComponent(() => import('../Editor/index.vue'));
const cinput = defineAsyncComponent(() => import('../Input/index.vue'));
const cselect = defineAsyncComponent(() => import('../Select/index.vue'));
const cstep = defineAsyncComponent(() => import('../Step/index.vue'));
const ctext = defineAsyncComponent(() => import('../Text/index.vue'));
const cselection = defineAsyncComponent(() => import('../Selection/index.vue'));
const ctitle = defineAsyncComponent(() => import('../Text/Title/index.vue'));
const csearch = defineAsyncComponent(() => import('../Searching/index.vue'));
const cbutton = defineAsyncComponent(() => import('../Button/index.vue'));
const clegend = defineAsyncComponent(() => import('../Legend/index.vue'));
const crefresh = defineAsyncComponent(() => import('../Refresh/index.vue'));
const crange = defineAsyncComponent(() => import('../Slider/index.vue'));
const ctransform = defineAsyncComponent(() => import('../TransForm/index.vue'));
const ctabs = defineAsyncComponent(() => import('../Tabs/index.vue'));
const ctreeSelect = defineAsyncComponent(() =>
  import('../TreeSelect/index.vue'),
);
const ctableFilter = defineAsyncComponent(() =>
  import('../TableSelection/index.vue'),
);
const cheaderPagination = defineAsyncComponent(() =>
  import('../HeaderPagination/index.vue'),
);
const clabelTab = defineAsyncComponent(() => import('../labelTab/index.vue'));
const cslider = defineAsyncComponent(() => import('../Slider/index.vue'));
const ctslider = defineAsyncComponent(() => import('../tableSlider/index.vue'));
export default {
  name: 'FormGroup',
  ...
  methods: {
    ...
    allSteps(args) {
      ...
      this.finalList.forEach((item, index) => {
        if (this.typeChecking(item.type)) {
          Object.assign(
            res,
            this.refOpera(item.name || 'comp' + index, 'allSteps', args),
          );
        }
      });
      return res;
    ...
    getNames() {
      const res = [];
      this.finalList.forEach((item, index) => {
        const mid = this.typeChecking(item.type)
          ? this.refOpera('comp' + index, 'getNames')
          : false;
        if (mid) {
          res.push(...mid);
        }
    ...
    compChange(name, type) {
      return (res) => {
        if (this.typeChecking(type)) {
          this.filterProperty[name] = res;
        } else {
          this.formParam[name] = res;
        }
      };
    },
    ...
    createComp(h, list, val, pos) {
      ...
      const component = this.impling(val.type);
      const child = h(this.getComponent(component), variable);
      return child;
    },
    // cc
    getComponent(type) {
      switch (type) {
        ...
        case 'ctableFilter':
          return ctableFilter;
        case 'cheaderPagination':
          return cheaderPagination;
        case 'clabelTab':
          return clabelTab;
        case 'cslider':
          return cslider;
    ...
    checkOnlySearching(compList) {
      if (
        compList.length === 1 &&
        compList[0] &&
        compList[0].type.toLowerCase().match('search')
      ) {
        return true;
      }
      return false;
        ...
        'section',
        {
          class:
            ...
            (this.inrow
              ...
              : '') +
            this.className +
            (this.checkOnlySearching(compList) ? 'group__inrow-right' : ''),
        },
