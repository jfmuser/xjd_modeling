...
function combine(origin, extend) {
  ...
  for (const key in extend) {
    if (originKeys.indexOf(key) >= 0) {
      ...
      const hasObj = originKeys.find(
        (item) => typeof origin[item] === 'object' && !item.match('csv'),
      );
      if (!hasObj) {
        continue;
      }
...
const dataReport = {
  ...
  methods: {
    getNextProperty(propertys, upd = true) {
      ...
      for (let i = 0, l = keys.length; i < l; i++) {
        ...
        if (this.originProperty.indexOf(wholeKey) < 0) {
          const hasNextLevel =
            typeof propertys[wholeKey] === 'object' &&
            Object.keys(propertys[wholeKey]).find(
              (item) =>
                typeof propertys[wholeKey][item] === 'object' &&
                propertys[wholeKey][item] !== null &&
                'title' in propertys[wholeKey][item],
            );
    ...
    allSteps(args, comp) {
      ...
      let hasForm = this.currentList.find((item) => {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (val.type === 'form') {
          this.oldProperty.push({
            prop: this.refOpera('comp' + i, 'getCurrentProperty'),
            connectTo: [],
          });
    ...
    startAsyncRequest(args = this.currentArgs) {
      ...
      if (nextComp) {
        ...
        if (nextProperty === 'default') {
          if (
            ['chart'].indexOf(nextComp.compType) >= 0 &&
            args.imply.length > 0
          ) {
            this.currentComp = nextComp;
            this.currentCompProp = '';
            const checked = this.refOpera(
              nextComp.comp,
              'setImply',
              args.imply,
            );
          ...
          if (
            nextProperty !==
            this.refOpera(nextComp.comp, 'getCurrentProperty').toString()
          ) {
            this.refOpera(
              nextComp.comp,
              'linkageChange',
    ...
    getNames() {
      ...
      this.currentList.forEach((item, index) => {
        if (
          ['form', 'table', 'async', 'group', 'chart', 'tabs'].indexOf(
            item.type,
          ) >= 0
        ) {
