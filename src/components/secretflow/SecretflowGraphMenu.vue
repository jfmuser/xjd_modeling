            ...
            stencil = new Addon.Stencil({
                ...
                getDropNode: (draggingNode) => {
                    ...
                    const sameTypeNodes = nodes.filter(
                        (item) => {
                            console.log(item,item.data.component_module, result.data.component_module, result,'OOOOOAAAAAOOO');
                            if(!item.data.component_module) return false
                            
                            return item.data.component_module === result.data.component_module ||
                                dictionary.algorithm_En_self[item.data.component_module.slice(0, -2)] === result.data.component_module
                        },
                    );
                    ...
                    algorithmVersionList.forEach(alg => {
                        if (alg.module === result.data.label) {
