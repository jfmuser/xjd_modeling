...
<template>
    <TableContainer ref="TableContainerRef" :showFilter="false" @page-change="onPageChange">
        ...
        row.jobId }}
            ...
            <el-table-column prop="fStatus" fixed="right" label="操作" v-if="route.query.type == '3'">
                <template #default="{ row }"><el-button type="text" @click="downloadResult(row.jobId)">下载结果</el-button>
                </template>
            </el-table-column>
            <el-table-column prop="fStatus" fixed="right" label="操作" v-if="route.query.type == '1'">
                <template #default="{ row }"><el-button type="text" @click="openDownloadDialog(row)">下载结果</el-button>
                </template>
            </el-table-column>
    ...
    width="500px"
  >
       <el-select v-model="param.taskId" placeholder="选择要下载的算子" style="width: 240px">
      <el-option
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="downloadAlgCsvData">
          确定
        </el-button>
      </div>
    </template>
