...
const PRECOLORS = ['#3145A6', '#DEECFC', '#0EC7A5'];
...
function struct() {
  ...
  lay.min = lay.min || -1;
  lay.drawInstance =
    lay.drawInstance || new Correlation(lay.features, lay.correlations, lay);
...
class Correlation {
  ...
  getLongestFeatureStyle(textFontSize, longest) {
    ...
    const eachWidth =
      (this.lay.width -
        stylus.xlength -
        stylus.xlength * lay.featureDistance -
        textFontSize) /
      this.newFeatures.length;
    const fontSize = square.getSuitableFont(
      this.lay.$ctx,
      '-0.000000',
      eachWidth,
      TEXT_FAMILY,
    ...
    if (
      fontSize + this.betweenFont < textFontSize ||
      fontSize - this.betweenFont > textFontSize
    ) {
      const change = (fontSize - textFontSize) / 2;
      const finalChange =
        Math.abs(change) < this.betweenFont
          ? change < 0
            ? -this.betweenFont
            : this.betweenFont
          : change;
    ...
    for (let i = 0; i < rever.length; i++) {
      ...
      for (let j = 0; j < this.newFeatures.length; j++) {
        const finalPosX =
          x +
          distance.xlength +
          distance.xlength * lay.featureDistance +
          (j + 0.5) * eachWidth +
          textFont;
  ...
  checkTextPos() {
    ...
    const yForFeaturex =
      y +
      this.newFeatures.length * eachWidth +
      (distance.ylength * lay.featureDistance) / 2 +
      textFont;
    for (let i = 0; i < this.newFeatures.length; i++) {
      const xForFeaturex =
        x +
        distance.xlength +
        distance.ylength * lay.featureDistance +
        (i + 0.5) * eachWidth +
        textFont;
  ...
  checkShowing(newFeatures) {
    ...
    this.checkTextPos();
    const width =
      lay.distance.xlength * (1 + lay.featureDistance) +
      this.newFeatures.length * lay.eachWidth;
    const height =
      lay.distance.ylength * (1 + lay.featureDistance) +
      this.newFeatures.length * lay.eachWidth;
...
class SquareInfo {
  ...
  getColorForNum(num) {
    ...
    const eachChange = parseFloat(
      Math.floor(
        ((PRECOLORS.length - 1) / (this.RangeAxis().length - 1)) * 100,
      ) / 100,
    );
    const poinExchangeToColor = eachChange * range;
    const startColor = Layer.toRGBA(PRECOLORS[Math.floor(poinExchangeToColor)]);
    ...
    const colorS = startColor.replace('rgba(', '').replace(')', '').split(',');
    const colorE = endColor.replace('rgba(', '').replace(')', '').split(',');
    ...
    square.drawSquare(
      {
        props: {
          ...
          disable:
            parseFloat(this.content) > lay.max ||
            parseFloat(this.content) < lay.min,
          fontSize: lay.contentFont,
        },
        events: square.events,
      },
      lay,
      this.featureX + '_' + this.featureY,
    );
  }
}
