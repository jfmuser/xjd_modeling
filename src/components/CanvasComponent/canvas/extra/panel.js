...
function drawContainer(ctx, start, radius, width, gradient = true) {
  if (gradient) {
    ctx.strokeStyle = createGradient(
      ...
      [
        { pos: 0, color: 'rgba(73, 78, 206, 0.6)' },
        { pos: 1, color: '#24b68b' },
      ],
    );
    ctx.lineWidth = 10;
...
function drawDegree(
  ctx,
  bet = CON_TO_DEGREE,
  long = DEGREE_DISTANCE,
  toWord = DEGREE_TO_NUM,
  r,
  start,
  font,
) {
...
const path = function () {
  ...
  const distance =
    CON_TO_DEGREE + DEGREE_DISTANCE + DEGREE_TO_NUM + NUM_TO_BOTTOM;
  drawContainer(
    ctx,
    {
      x: lay.x + Math.sin(Math.PI / 4) * distance,
      y: lay.y + Math.cos(Math.PI / 4) * distance,
    },
    lay.r - distance,
    lay.width - Math.sin(Math.PI / 4) * distance * 2,
    false,
  );
