...
class DiagramInfo {
  ...
  checkNewPort(obj) {
    newPort =
      Object.keys(obj).length > 0
        ? !!obj[Object.keys(obj)[0]][0]['up_output_info']
        : true;
  }
  componentInfo(list) {
    ...
    for (const name in obj) {
      for (const item of obj[name]) {
        ...
        this.linking[item.type].push({
          ...
          outputType: item.up_output_info
            ? item.up_output_info.join('')
            : item.type + '0',
        });
  ...
  componentCheckWidthAndHeight() {
    ...
    const styleInfo = measureText(this.lay.$ctx, longest, {
      font: this.lay.fontSizeForContent + 'px ' + FONT_FAMILY,
    });
  ...
  checkPos() {
    ...
    const getInList = function (topto, size, index) {
      ...
      return {
        x: firstX + index * (width + lay.componentBetween),
        y: index * (height + lay.innerBetween) + topto + height / 2,
      };
    };
  ...
  getLinking() {
    ...
    for (const key in this.linking) {
      ...
      for (const val of this.linking[key]) {
        ...
        link.line.push(
          this.lay.$children
            .get(val.components[0])
            .$meta.get('port')
            .get(val.outputType + 'output'),
        );
        link.line.push(
          this.lay.$children
            .get(val.components[1])
            .$meta.get('port')
            .get(key + 'input'),
        );
        ...
        link.name =
          p.name + '|' + c.name + '|' + val.outputType + '|' + key + '|line';
        ...
        Layer.component.line.drawLine(
          {
            props: {
              ...
              style: {
                lineWidth:
                  this.lay.lineWidthForLinking *
                  (this.lay._lineBright && val.name.match(this.lay._lineBright)
                    ? 1.5
                    : 1),
                strokeStyle:
                  this.lay._lineBright && val.name.match(this.lay._lineBright)
  ...
  getStyleOfDag() {
    ...
    for (const item of this.level) {
      ...
      dagHeight +=
        item.length * this.compHeight +
        (item.length - 1) * this.lay.innerBetween +
        this.lay.levelBetween;
    }
    dagHeight -= this.lay.levelBetween;
    dagWidth =
      longest * this.compWidth + (longest - 1) * this.lay.componentBetween;
...
const flowDiagram = {
  ...
  events: {
    ...
    toSuccess(name, img) {
      const lay = this;
      Progress.animations.toNewType.call(
        lay.$children.get(name),
...
class CompExpression {
  ...
  setImage(imgs) {
    this.img = imgs.get(
      (this.disable ? 'DISABLE_' : '') + this.type.split('|')[0],
    );
  }
  ...
  setPort(ports, dep) {
    ...
    if (
      !this.belone
        .toLowerCase()
        .match(
          /(intersection|federatedsample|evaluation|upload|download|rsa|datasplit|reader|union|scorecard|sampleweight)/i,
        )
    ) {
    ...
    Progress.drawProgress(
      {
        props: {
          ...
          width: this.width,
          height: this.height,
          type: lay.$children.get(this.name)
            ? lay.$children.get(this.name).type
            : this.type,
          ...
          time: lay.$children.get(this.name)
            ? lay.$children.get(this.name).time
            : this.time,
          img: this.img,
        },
        zindex: 1,
