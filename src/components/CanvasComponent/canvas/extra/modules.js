...
const progressComp = {
  drawProgress(obj, parent, name) {
    ...
    if (parent) {
      ...
      if (
        newLayer &&
        progressComp.type.RUNNING.match(currentlay.type.toUpperCase())
      ) {
  ...
  events: {
    choose(point, afterChoose, ...props) {
      ...
      lay.setCus('choose', () => {
        lay.containerStyle = _getContainerColor(
          lay.type,
          !!lay.choose,
          lay.disable,
        );
        lay.contentStyle = _getContainerColor(
          lay.type,
          !!lay.choose,
          lay.disable,
          'content',
        ...
        if (afterChoose && typeof afterChoose === 'function')
          afterChoose(lay.text, lay.$here(point));
      });
    },
    linkChoose(point, afterChoose, ...props) {
      ...
      if (lay.$here(point)) {
        lay.setCus('choose', () => {
          if (afterChoose && typeof afterChoose === 'function')
            afterChoose(lay.$parent.text, lay.$here(point), true);
        });
        return 'finish';
      }
    },
    scale(time, point = { x: 0, y: 0 }) {
      ...
      lay.setCus('scale', () => {
        ...
        lay.betweenIconWithContent = Layer.toFixed(
          lay.betweenIconWithContent * time,
        );
        ...
        lay.betweenPortWidthTooltip = Layer.toFixed(
          lay.betweenPortWidthTooltip * time,
        );
  ...
  animations: {
    ...
    loading() {
      ...
      changeList.push(
        lay.settingCus(() => {
          lay.progress = 0;
          lay.contentStyle = _getContainerColor(
            lay.type || progressComp.type.UNRUN,
            !!lay.choose,
            lay.disable,
            'content',
          );
    ...
    toNewType(type, timeOrPic) {
      ...
      if (type !== progressComp.type.RUNNING) {
        ...
        changeList.toNewTypeContent.push(
          lay.settingNum(
            lay.progress || 1,
            1,
            (to) => {
      ...
      const toColor = _getContainerColor(
        type,
        !!lay.choose,
        lay.disable,
        'content',
      ...
      changeList.toNewTypeContent.push(
        lay.settingCus(() => {
          ...
          if (
            type === progressComp.type.SUCCESS ||
            type === progressComp.type.FAIL
          ) {
            lay.img = timeOrPic;
          ...
          if (
            type === progressComp.type.FAIL &&
            !lay.$children.get('retryBtn').$visiable
          ) {
      ...
      const toContainerColor = _getContainerColor(
        type,
        !!lay.choose,
        lay.disable,
      );
      changeList.toNewTypeContainer.push(
        lay.settingRGBA(
          lay.containerStyle,
          toContainerColor,
          (to) => {
            lay.containerStyle = to;
          },
          200,
        ),
      );
      const toFontStyle = _getTextColor(type, !!lay.choose, lay.disable);
      changeList.toNewTypeText.push(
        lay.settingRGBA(
          lay.fontStyle,
          toFontStyle,
          (to) => {
...
function getStyle(type, choose, disable, lay) {
  if (lay) {
    lay.containerStyle =
      lay.containerStyle ||
      _getContainerColor(type || progressComp.type.UNRUN, !!choose, disable);
    lay.contentStyle =
      lay.contentStyle ||
      _getContainerColor(
    ...
    lay.fontStyle =
      lay.fontStyle ||
      _getTextColor(type || progressComp.type.UNRUN, !!choose, disable);
  }
  const style = {
    container: {
      fillStyle: lay
...
function struct() {
  ...
  lay.betweenIconWithContent =
    lay.betweenIconWithContent || BETWEEN_ICON_WITH_CONTENT;
  lay.lineWidth = lay.lineWidth || LINEWIDTH;
  lay.portWidth =
    lay.portWidth || (!lay.specialDataInput ? PORT_WIDTH : SPEC_PORT_WIDTH);
  ...
  drawingStyle.container.fillStyle =
    lay.containerLightUp || drawingStyle.container.fillStyle;
  const w =
    lay.width ||
    Math.ceil(fontStyle.width) + lay.padding * 2 + lay.lineWidth * 2;
      ...
      Layer.component.tooltip.drawTooltip(
        {
          props: {
            point: {
              x: point.x,
              y: point.y - lay.portHeight / 2 - lay.betweenPortWidthTooltip,
            },
