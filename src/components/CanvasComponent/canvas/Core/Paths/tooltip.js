...
function struct() {
  ...
  const contentHeight =
    parseInt(lay.textStyle.font) * lay.text.length +
    lay.linebetween * (lay.text.length - 1) +
    lay.padding * 1.5;
  ...
  for (let i = 0; i < lay.text.length; i++) {
    ...
    y =
      y -
      ((lay.text.length - 1) / 2 - i) *
        (parseInt(lay.textStyle.font) + lay.linebetween);
    Text.drawText(
      {
        props: {
          text: lay.text[i],
  ...
  lay.$meta.set('clear', {
    width: contentWidth,
    height: contentWidth + lay.trangleSize,
  });
}
