...
export function text() {
  ...
  const breakLine = lay.breakLine
    ...
    : 1;
  const finalLine =
    lay.width && Math.ceil(parseInt(textInfo.width) / lay.width) < breakLine
  ...
  for (let i = 0; i < finalLine; i++) {
    let lengthCheck = Math.floor(
      (lay.text.length * (lineWidth * breakLine)) / parseInt(textInfo.width),
    );
    while (
      parseFloat(
        measureText(ctx, lay.text.substr(pos, lengthCheck), lay.style).width,
      ) > lineWidth
    ) {
      lengthCheck--;
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ctx.fillText(
      lines[i],
      x,
      y + i * lineHeight + (lines.length > 1 ? lay.betweenEachLine : 0),
    );
  }
  ctx.restore();
}
...
export function measureText(ctx, text, style, angle = 0) {
  ...
  final.xlength =
    Math.sin(angle) !== 0 ? final.width * Math.cos(angle) : final.width;
  ctx.restore();
  return final;
}
