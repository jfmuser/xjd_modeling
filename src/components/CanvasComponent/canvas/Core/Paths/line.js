...
export function calculation(startP, endP, horizen, levelBetween, max, min) {
  ...
  if (horizen || (startP.x !== endP.x && startP.y !== endP.y)) {
    ...
    const ps = {
      x: sx,
      y: ey > sy ? sy + distanceRate * distance : sy - distanceRate * distance,
    };
    const pe = {
      x: ex,
      y: ey > sy ? ey - distanceRate * distance : ey + distanceRate * distance,
    };
    finalPoints.push(ps);
    if (horizen && sx === ex) {
      if (horizen === 'left') {
