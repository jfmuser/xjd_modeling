...
export default function InitTranslate(Layer) {
    ...
    if (translate) {
      for (const key in translate) {
        if (Array.isArray(translate[key])) {
          ...
          if (
            typeof translate[key].repeat === 'boolean' &&
            translate[key].steps
          ) {
            this.registerChainTranslate(
              translate[key].repeat,
              ...translate[key].steps,
            );
          } else {
            this.registerChainTranslate(false, translate[key]);
  ...
  Layer.prototype.settingRGBA = function (
    from,
    to,
    setting,
    costTime,
    ...
    const final = {
      ...
      origin: from,
      process() {
        const origin = toRGBA(this.origin)
          .replace('rgba(', '')
          .replace(')$', '')
          .split(',');
        const target = toRGBA(this.target)
          .replace('rgba(', '')
          .replace(')$', '')
          .split(',');
        const current = toRGBA(this.current)
          .replace('rgba(', '')
          .replace(')$', '')
          .split(',');
        for (let i = 0; i < origin.length; i++) {
          const once =
            (parseFloat(target[i]) - parseFloat(origin[i])) /
            Math.floor(
              (costTime || intervalTime) / (betweenTime || intervalTime),
            );
          current[i] = parseFloat(current[i]) + once;
          if (
            (once < 0 && current[i] < parseFloat(target[i])) ||
            (once > 0 && current[i] > parseFloat(target[i]))
          ) {
        ...
        return 'rgba(' + current.join(',') + ')' === 'rgba(' + target.join(',')
          ? true
          : 'rgba(' + current.join(',') + ')';
      },
    };
  ...
  Layer.prototype.settingNum = function (
    from,
    to,
    setting,
    costTime,
  ...
  Layer.prototype.setCus = function (cusname, process, costTime) {
    ...
    this.registerChainTranslate(
      name,
      false,
      this.settingCus(process, costTime),
    );
    return name;
  };
