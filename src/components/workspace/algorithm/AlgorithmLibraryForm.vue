...
const rules = reactive({
  name: [
    { required: true, message: '请填写算法库名', trigger: 'blur' },
...
<template>
  <el-form ref="FormRef"
    ...
    <el-form-item label="版本号"
                  prop="outerVerNo">
      <el-input v-model="state.model.outerVerNo"
                placeholder="请输入版本号，格式为x.x.x"
                :disabled="props.formType === FormType.READ"></el-input>
    </el-form-item>
    <el-form-item v-show="false"
                  label="最低版本号"
                  prop="minVerNo">
      <el-select v-model="state.model.minVerNo"
                 placeholder="请选择最低版本号"
                 :disabled="props.formType === FormType.READ">
        <el-option v-for="item in state.verList"
                   :key="item.innerVerNo"
                   :label="item.outerVerNo"
                   :value="item.innerVerNo" />
      </el-select>
    </el-form-item>
    <el-form-item label="发布状态"
                  prop="publishState">
      <el-select v-model="state.model.publishState"
                 :disabled="true">
        <el-option v-for="item in AlgorithmLibPublishType.options"
                   :key="item.value"
                   :label="item.label"
                   :value="item.value" />
      </el-select>
    ...
    <el-form-item v-if="props.formType === FormType.IMPORT"
                  label="配置文件">
      <el-button type="success"
                 @click="clickLoad">点击选择</el-button>
      <input ref="RefFile"
             type="file"
             accept=".json"
             style="display: none"
             @change="fileLoad" />
      <span v-if="state.model.file"
